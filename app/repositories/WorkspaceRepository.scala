...
import models.tables.{WorkspaceTable, UserWorkspaceTable}
...
class WorkspaceRepository @Inject() (
    ...
    def getWorkspacesForUserQuerySimple(userId: Int): Query[WorkspaceTable, Workspace, Seq] = {
        workspaces
            .join(userWorkspaces)
    ...
    def getWorkspaceForUserQuery(workspaceId: Int, userId: Int): Query[WorkspaceTable, Workspace, Seq] = {
        workspaces
            .join(userWorkspaces)
            .on(_.id === _.workspaceId)
            .filter { case (workspace, userWorkspace) =>
...
//                && userWorkspace.userId === userId
    ...
    def deleteWorkspaceForUserAction(workspaceId: Int, userId: Int): DBIO[Int] = {
        ...
        val hasAdminAccess = userWorkspaces
            .filter(uw => uw.workspaceId === workspaceId &&
                uw.userId === userId
//                && uw.role === Enums.UserWorkspaceRole.admin
        ...
        for {
            ...
            deleteCount <- if (hasAccess) {
                ...
                userWorkspaces.filter(_.workspaceId === workspaceId).delete.andThen(
